...
import { ArrowRightIcon, ChevronDownIcon, ArrowPathIcon  } from "@heroicons/react/16/solid";
...
export default function Home() {
  ...
  const [selectedCarType, setSelectedCarType] = useState<{ name: string; id: number } | null>(null);
  ...
  useEffect(() => {
    const fetchCarTypes = async () => {
      try {
        ...
        const sortedTypes = result.Results.sort((a: { MakeName: string; }, b: { MakeName: string; }) => a.MakeName.localeCompare(b.MakeName));
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24 bg-pageBg ">
      <div className="bg-white shadow-searchBox p-10 rounded-md ">
        <p className="text-3xl font-bold text-black mb-5 text-center">Search For your Car</p>
        ...
        <div className="flex items-center justify-left gap-5">
                ...
                <Menu as="div" className="relative inline-block text-left w-72">
                  {loading ? <p className="text-center text-black">Loading...</p> : (
                    <>
                     ...
                     <MenuItems transition className="origin-top-right absolute right-0 mt-2 w-72 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none overflow-y-auto h-60">
                     
                    
                     {carTypes.map((carType: any) => (
                       <MenuItem key={carType.MakeId}>
                       <button onClick={() => {selectedCarType === carType.MakeId ? setSelectedCarType(null) : setSelectedCarType({name: carType.MakeName, id: carType.MakeId})}} className="group flex w-full items-center gap-2 rounded-lg py-1.5 px-3 data-[focus]:bg-black/10 text-black">
                        {carType.MakeName}
                        </button>
                      </MenuItem>
                     ))}
          ...
          {years.map((year: any) => (
            <MenuItem key={year}>
            <button onClick={() => {selectedYear === year ? setSelectedYear(null) : setSelectedYear(year)}} className="group flex w-full items-center gap-2 rounded-lg py-1.5 px-3 data-[focus]:bg-black/10 text-black">
             {year}
             </button>
           </MenuItem>
          ))}
